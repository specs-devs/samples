  ...
  */
  ...
  private init() {
    ...
    this._script.createEvent("OnStartEvent").bind(() => {
      const camID = this.isEditor ? CameraModule.CameraId.Default_Color : CameraModule.CameraId.Default_Color
  ...
  private setUpVirtualCamera(cameraType: CameraType, trackingCam: DeviceCamera) {
    const cameraObj: SceneObject = global.scene.createSceneObject("SpecsLeftCamera")
    const cameraComponent: Camera = cameraObj.createComponent("Camera")
    ...
    //set pose
    const camTrans = cameraObj.getTransform()
