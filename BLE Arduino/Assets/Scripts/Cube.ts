...
export class Cube extends BaseScriptComponent {
  ...
  private softFollow() {
    const desiredPosition = this.camTrans.getWorldPosition().add(this.camTrans.forward.uniformScale(-CAM_DISTANCE))

    this.trans.setWorldPosition(
      vec3.lerp(this.trans.getWorldPosition(), desiredPosition, getDeltaTime() * FOLLOW_SPEED)
    )
  }
}
