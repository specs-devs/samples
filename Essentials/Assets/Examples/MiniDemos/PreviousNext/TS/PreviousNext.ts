...
export class PreviousNext extends BaseScriptComponent {
  ...
  onStart() {
    ...
    if (this.nextInteractable) {
      this.nextInteractable.onInteractorTriggerStart.add(onTriggerStartCallbackNext)
    } else {
      this.logger.warn("Next interactable is not defined.")
    }

    if (this.previousInteractable) {
      this.previousInteractable.onInteractorTriggerStart.add(onTriggerStartCallbackPrevious)
    } else {
