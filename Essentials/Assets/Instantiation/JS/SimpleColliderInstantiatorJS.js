...
function SimpleColliderInstantiator() {
  ...
  function randomPointInBox() {
    ...
    print(
      "DEBUG: Using object scale as box size: " +
        boxSize.x.toFixed(2) +
        ", " +
        boxSize.y.toFixed(2) +
        ", " +
  ...
  function randomPointOnBoxSurface() {
    ...
    print(
      "DEBUG: Using object scale as box size for surface: " +
        boxSize.x.toFixed(2) +
        ", " +
        boxSize.y.toFixed(2) +
        ", " +
  ...
  function randomPointInSphere() {
    ...
    randomPoint = new vec3(x * normalizedDistance, y * normalizedDistance, z * normalizedDistance);

    print(
      "DEBUG: Generated sphere point at local coordinates (" +
        randomPoint.x.toFixed(2) +
        ", " +
        randomPoint.y.toFixed(2) +
        ", " +
        randomPoint.z.toFixed(2) +
        ")"
    );
  ...
  function randomPointOnSphereSurface() {
    ...
    var randomPoint = new vec3(x * normalizedDistance, y * normalizedDistance, z * normalizedDistance);

    print(
      "DEBUG: Generated sphere surface point at local coordinates (" +
        randomPoint.x.toFixed(2) +
        ", " +
        randomPoint.y.toFixed(2) +
        ", " +
  ...
  function getCapsulePropertiesFromTransform() {
    ...
    print(
      "DEBUG: Derived capsule properties from transform - radius: " +
        radius.toFixed(2) +
        ", length: " +
        length.toFixed(2) +
        ", axis: " +
  ...
  function transformPoint(localPoint, position, rotation) {
    ...
    // Apply translation
    var worldPoint = new vec3(position.x + rotatedPoint.x, position.y + rotatedPoint.y, position.z + rotatedPoint.z);
    ...
    print(
      "DEBUG: Local point " +
        ...
        " transformed to world point " +
        worldPoint.x.toFixed(2) +
        ", " +
        worldPoint.y.toFixed(2) +
        ", " +
        worldPoint.z.toFixed(2)
    );
  ...
  function rotatePointByQuaternion(point, rotation) {
    ...
    var qx =
      point.x * (1 - 2 * y * y - 2 * z * z) + point.y * (2 * x * y - 2 * w * z) + point.z * (2 * x * z + 2 * w * y);
    var qy =
      point.x * (2 * x * y + 2 * w * z) + point.y * (1 - 2 * x * x - 2 * z * z) + point.z * (2 * y * z - 2 * w * x);
    var qz =
      point.x * (2 * x * z - 2 * w * y) + point.y * (2 * y * z + 2 * w * x) + point.z * (1 - 2 * x * x - 2 * y * y);
  ...
  function createPrefabInstance(position) {
    if (script.prefab) {
      ...
      print(
        "Created prefab instance at position: " +
          instancePosition.x.toFixed(2) +
          ", " +
          instancePosition.y.toFixed(2) +
          ", " +
          instancePosition.z.toFixed(2) +
          " with scale: " +
          instance.getTransform().getLocalScale().x.toFixed(2)
      );
    }
  }
}
