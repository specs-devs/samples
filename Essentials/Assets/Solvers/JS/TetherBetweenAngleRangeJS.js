...
function TetherBetweenAngleRangeJS() {
  ...
  function evaluateConditions() {
    ...
    var targetToObject = new vec3(myPos.x - targetPos.x, myPos.y - targetPos.y, myPos.z - targetPos.z);

    // Calculate distances
    _verticalDistance = Math.abs(targetToObject.y);
    _horizontalDistance = Math.sqrt(targetToObject.x * targetToObject.x + targetToObject.z * targetToObject.z);
    ...
    if (script.showDebug) {
      ...
      print(
        "VertDist: " + _verticalDistance.toFixed(1) + " > " + script.verticalDistanceThreshold + "? " + verticalTrigger
      );
      print(
        "HorizDist: " +
          _horizontalDistance.toFixed(1) +
          " > " +
          script.horizontalDistanceThreshold +
          "? " +
          horizontalTrigger
      );
  ...
  function updateContentPosition() {
    ...
    var newPos = lerpVector(currentPos, _targetPosition, script.lerpSpeed * getDeltaTime());
  ...
  script.getCurrentAngle = function () {
    return _currentAngle;
  };
  script.getHorizontalDistance = function () {
    return _horizontalDistance;
  };
  script.getVerticalDistance = function () {
