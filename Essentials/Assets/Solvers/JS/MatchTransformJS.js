...
function MatchTransformJS() {
  ...
  function updatePosition(myTransform, targetTransform) {
    ...
    if (script.usePositionLerp) {
      ...
      newPosition = lerpVector(currentPosition, newPosition, script.positionLerpSpeed * getDeltaTime());
    } else {
  ...
  function updateRotation(myTransform, targetTransform) {
    ...
    // Apply lerp
    var lerpedRotation = quat.slerp(currentRotation, newRotation, script.rotationLerpSpeed * getDeltaTime());
  ...
  function updateScale(myTransform, targetTransform) {
    ...
    // Apply lerp
    var lerpedScale = lerpVector(currentScale, newScale, script.scaleLerpSpeed * getDeltaTime());
