...
export class AIAssistantUIBridge extends BaseScriptComponent {
  ...
  // Captured at start so the buttons and hints can be restored when the user
  // closes the assistant and the picker reappears.
  private geminiButtonScale: vec3
  private openAIButtonScale: vec3
  private initialHintTitle: string
  private initialHintText: string
  // Event wiring guards: startWebsocketAndUI can run once per picker round,
  // but these subscriptions live on component-level events and must not stack.
  private connectedAssistants = new Set<GeminiAssistant | OpenAIAssistant>()
  private activationWired: boolean = false
  ...
  private onStart(): void {
    ...
    this.geminiButtonScale = this.geminiButton.sceneObject.getTransform().getLocalScale()
    this.openAIButtonScale = this.openAIButton.sceneObject.getTransform().getLocalScale()
    this.initialHintTitle = this.hintTitle.text
    this.initialHintText = this.hintText.text

    this.sphereController.onCloseButtonEvent.add(() => {
      this.closeAssistantAndShowButtons()
    })
    // SpectaclesUIKit buttons whose script reference can come through
    // unresolved after the package update (initialize()/onTriggerUp undefined)
    // until reattached in the scene. Guard so the lens does not crash; once
    // reattached these wire up automatically.
    const geminiBtn = this.geminiButton as any
    if (typeof geminiBtn.initialize === "function") geminiBtn.initialize()
    geminiBtn.onTriggerUp?.add(() => {
      this.assistantType = AssistantType.Gemini
      this.hintTitle.text = "Gemini Live Example"
      this.startWebsocketAndUI()
    })

    const openAIBtn = this.openAIButton as any
    if (typeof openAIBtn.initialize === "function") openAIBtn.initialize()
    openAIBtn.onTriggerUp?.add(() => {
  ...
  private closeAssistantAndShowButtons(): void {
    if (this.currentAssistant) {
      this.currentAssistant.closeSession()
    }
    this.sphereController.hideUI()
    this.hintTitle.text = this.initialHintTitle
    this.hintText.text = this.initialHintText
    this.showButtons()
  }

  private showButtons(): void {
    this.restoreButton(this.geminiButton, this.geminiButtonScale)
    this.restoreButton(this.openAIButton, this.openAIButtonScale)
  }

  private restoreButton(button: BaseButton, originalScale: vec3): void {
    button.sceneObject.enabled = true
    button.enabled = true
    ...
    animate({
      ...
      update: (t) => {
        tr.setLocalScale(new vec3(
          MathUtils.lerp(start.x, originalScale.x, t),
          MathUtils.lerp(start.y, originalScale.y, t),
          MathUtils.lerp(start.z, originalScale.z, t)
        ))
      }
    })
  }
  ...
  private startWebsocketAndUI(): void {
    ...
    if (!this.activationWired) {
      this.activationWired = true
  ...
  private connectAssistantEvents(): void {
    if (this.connectedAssistants.has(this.currentAssistant)) return
    this.connectedAssistants.add(this.currentAssistant)
