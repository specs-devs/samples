...
export class ASRQueryController extends BaseScriptComponent {
  ...
  private init(): void {
    ...
    // The button is a SpectaclesUIKit button whose script reference can come
    // through unresolved after the package update (initialize()/onTriggerUp
    ...
    const queryBtn = this.button as any
    if (typeof queryBtn.initialize === "function") queryBtn.initialize()
    queryBtn.onTriggerUp?.add(() => {
  ...
  public getVoiceQuery(): Promise<string> {
    return new Promise((resolve, reject) => {
      if (this.isRecording) {
        PlaygroundLogger.log("ASR recording canceled (pinched while listening)")
      ...
      PlaygroundLogger.log("ASR listening... (speak, then pause to finish)")
      ...
      asrSettings.onTranscriptionUpdateEvent.add((asrOutput) => {
        if (asrOutput.isFinal) {
          PlaygroundLogger.log(`ASR heard: "${asrOutput.text}"`)
          this.isRecording = false
          this.animateVoiceIndicator(false)
          this.asrModule.stopTranscribing()
      ...
      asrSettings.onTranscriptionErrorEvent.add((asrOutput) => {
        PlaygroundLogger.log(`ASR error: ${asrOutput}`)
        this.isRecording = false
        this.animateVoiceIndicator(false)
        reject(asrOutput)
      })
