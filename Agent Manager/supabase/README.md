...
Supabase acts as the relay between the Specs Lens and the local Bridge daemon. It handles:
...
Install the Supabase CLI and log in to Snap Cloud first. The `--profile snap` flag is required — without it you are logged in to public supabase.com, not Snap Cloud:
...
This creates (or links) your Snap Cloud project, pushes the database schema, sets secrets, deploys edge functions, and writes `Bridge/.env` with your project URL and anon key. Answer **Y** to its prompts — the defaults are correct.

Useful locations in the [cloud.snap.com](https://cloud.snap.com) dashboard:
...
All commands must use the snap profile:

```bash
supabase link --project-ref <project-id> --profile snap --dns-resolver https
supabase db push --profile snap --dns-resolver https
supabase secrets set KEY_ENC_KEY=... WEBHOOK_SECRET=... --profile snap
supabase functions deploy --no-verify-jwt --profile snap
```
