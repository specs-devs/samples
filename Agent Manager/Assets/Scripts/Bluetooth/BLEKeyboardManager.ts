...
export class BLEKeyboardManager extends BaseScriptComponent {
  ...
  // Loaded lazily: accessing the Bluetooth module triggers the OS privacy
  // gating that disables camera frame / audio access in the lens (unless
  // Extended Permissions is enabled). Only pay that cost if the in-lens BLE
  // flow is actually used — never at component creation.
  private _bluetoothModule: Bluetooth.BluetoothCentralModule | null = null;
  private get bluetoothModule(): Bluetooth.BluetoothCentralModule {
    if (!this._bluetoothModule) {
      this._bluetoothModule = require("LensStudio:BluetoothCentralModule");
    }
    return this._bluetoothModule;
  }
  ...
  async connectToDevice(address: Uint8Array, name: string): Promise<void> {
    ...
    try {
      ...
      // NOTE: The BLE keyboard is now paired/connected at the system level, so the
      // explicit GATT connection-state event handling has been removed (the
      // `gatt.onConnectionStateChangedEvent` API and `Bluetooth.ConnectionState`
      // enum no longer exist in the current Bluetooth module). Because
      // `connectGatt` resolving already means we have a live connection, we
      // register notifications directly here. Disconnects are handled manually
      // via `unpairDevice()`.
      ...
      // Not every characteristic supports notify — register per-characteristic
      // so one unsupported characteristic doesn't abort the whole connection.
      let charCount = 0;
      for (const service of gatt.getServices()) {
        ...
        for (const char of service.getCharacteristics()) {
          ...
          try {
            ...
            print(`[BLE] notifications unsupported on ${char.uuid}: ${e}`);
          }
        }
      }
