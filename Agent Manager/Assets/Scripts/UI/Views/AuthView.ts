...
export class AuthView {
  ...
  initialize(): void {
    ...
    // While the system keyboard is open, stop the panel from capturing
    // far-field rays so pinch targeting reaches the keyboard keys.
    this.inputField.onKeyboardStateChanged.add((isOpen: boolean) => {
      this.setPanelRayTargetsEnabled(!isOpen);
    });
  ...
  private setPanelRayTargetsEnabled(enabled: boolean): void {
    if (isNull(this.plateObj)) return;
    const interactable = this.plateObj.getComponent(
      Interactable.getTypeName(),
    ) as Interactable | null;
    if (interactable) interactable.enabled = enabled;
    const plane = this.plateObj.getComponent(
      InteractionPlane.getTypeName(),
    ) as InteractionPlane | null;
    if (plane) plane.enabled = enabled;
    const collider = this.plateObj.getComponent(
