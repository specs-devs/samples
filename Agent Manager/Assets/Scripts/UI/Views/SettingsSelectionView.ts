...
export class SettingsSelectionView {
  ...
  showToggleList(title: string, entries: ToggleEntry[]): void {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      // Must go through addObject like the rows above — children parented
      // directly to the ScrollWindow object do not move with the scroller.
      const descRowObj = global.scene.createSceneObject(`ToggleDescRow_${i}`);
      if (useScroll) {
        this.scrollWindow!.addObject(descRowObj);
      } else {
        descRowObj.setParent(this.contentContainer);
      }
      descRowObj.getTransform().setLocalPosition(new vec3(0, cursorY, 0));
      createText({
        parent: descRowObj,
        name: `ToggleDesc_${i}`,
      ...
      this.contentObjects.push(descRowObj);
      cursorY -= TOGGLE_DESC_HEIGHT / 2;
  ...
  private measureDisclaimerHeight(
    ...
    const probeText = createText({
      ...
      horizontalOverflow: HorizontalOverflow.Wrap,
      horizontalAlignment: HorizontalAlignment.Left,
      layoutRect: Rect.create(-innerWidth / 2, innerWidth / 2, -50, 50),
    });
