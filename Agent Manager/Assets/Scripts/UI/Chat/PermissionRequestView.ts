...
export class PermissionRequestView {
  ...
  show(
    ...
    const toolText = createText({
      ...
      layoutRect: Rect.create(-textWidth / 2, textWidth / 2, -50, 50),
    });
  ...
  private completeLayout(): void {
    ...
    // Position desc text directly in the container — avoids layoutRect
    ...
    descText.layoutRect = Rect.create(
      -textWidth / 2,
      textWidth / 2,
      -visibleDescHeight / 2,
      visibleDescHeight / 2,
    ...
    const footerText = createText({
      ...
      horizontalAlignment: HorizontalAlignment.Left,
      layoutRect: Rect.create(-width / 2, width / 2, -50, 50),
    });
