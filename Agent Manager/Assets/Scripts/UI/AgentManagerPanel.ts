...
import { CONTENT_TILT_DEG, PANEL_WIDTH, Z_CONTENT } from "./Shared/UIConstants";
...
// The HorizontalFrame billboards its plate at CONTENT_TILT_DEG (-75°, nearly
// flat). Content counter-rotates by the exact opposite so it stands vertical
// on the flat plate — keep these in sync via the shared constant.
const CONTENT_COUNTER_TILT_DEG = -CONTENT_TILT_DEG;
...
export class AgentManagerPanel extends BaseScriptComponent {
  ...
  private onStart(): void {
    ...
    this.contentRoot
      .getTransform()
      .setLocalRotation(
        quat.fromEulerAngles(MathUtils.DegToRad * CONTENT_COUNTER_TILT_DEG, 0, 0),
      );
