...
export class SupabaseService {
  ...
  // Lazily (re-)establishes the user session. Deduplicates concurrent
  // callers so a burst of requests triggers a single sign-in.
  private signInPromise: Promise<boolean> | null = null;
  ...
  async invokeFunction<T = unknown>(
    ...
    // Edge functions resolve the user from the session token; calling without
    // one yields an opaque 401. If startup sign-in failed (e.g. network not
    // ready at lens launch), retry it here instead of failing the call.
    const signedIn = await this.ensureSignedIn();
    if (!signedIn) {
      throw new Error(
        `${TAG} Function ${functionName} skipped: not signed in to Snap Cloud (check internet connection, then retry)`,
      );
    }
