...
function setupBoundsCheck() {
  ...
  whenValueBecomes(
    function () {
      ...
      return (
        localPos.x >= aabbMin.x &&
        localPos.x <= aabbMax.x &&
        localPos.y >= aabbMin.y &&
        localPos.y <= aabbMax.y &&
        localPos.z >= aabbMin.z &&
        localPos.z <= aabbMax.z
      );
    },
    valueToCheck,
    onTrigger,
...
function setupDistanceCheck() {
  ...
  var flattenZ =
    script.distanceCheckFlattenZForScreenTransforms &&
    script.distanceCheckObjectA.getComponent("Component.ScreenTransform") &&
    script.distanceCheckObjectB.getComponent("Component.ScreenTransform");
  var distFunc = flattenZ
    ? function () {
...
function setupPhysicsColliderEvent() {
  ...
  var collisionFilter = makeObjectFilter(
    script.physicsColliderEventFilterObjectsBy,
    script.physicsColliderEventAllowedObjects,
    script.physicsColliderEventNameMatchType,
    script.physicsColliderEventAllowedNames,
...
function setupPhysicsRaycast() {
  ...
  var collisionFilter = makeObjectFilter(
    script.physicsRaycastFilterObjectsBy,
    script.physicsRaycastAllowedObjects,
    script.physicsRaycastNameMatchType,
    script.physicsRaycastAllowedNames,
    "Physics.ColliderComponent"
  );
...
function triggerAnimateMeshAnimationPlayer() {
  ...
  var animationClip = script.animateMeshAnimationPlayerAnimationPlayer.getClip(
    script.animateMeshAnimationPlayerClipName
  );
  if (!animationClip) {
  ...
  switch (script.animateMeshAnimationPlayerAction) {
    case "Play":
      script.animateMeshAnimationPlayerAnimationPlayer.playClipAt(
        script.animateMeshAnimationPlayerClipName,
        animationClip.begin
      );
      break;
    ...
    default:
      if (
        script.animateMeshAnimationPlayerAnimationPlayer.getClipIsPlaying(script.animateMeshAnimationPlayerClipName)
      ) {
  ...
  switch (script.playSoundAction) {
    ...
    default:
      ...
      if (
        !(
          provider &&
          (provider.isOfType("Provider.FileLicensedSoundProvider") ||
            provider.isOfType("Provider.StudioLicensedSoundProvider"))
        )
      ) {
...
function triggerSetRotation() {
  ...
  setTranRot(
    tran,
    quat.fromEulerVec(script.setRotationRotationAngle.uniformScale(Math.PI / 180)),
  ...
  switch (script.callScriptAPICallType) {
    ...
    default:
      ...
      if (targetAPI[script.callScriptAPIFunctionName]) {
        ...
        debugPrint(
          script.callScriptAPITargetType + " missing function named '" + script.callScriptAPIFunctionName + "'"
        );
      }
      break;
    case 1:
...
function triggerPhysicsApplyForce() {
  ...
  switch (script.physicsApplyForceSpace) {
    ...
    case "Local to Object":
      ...
      forceToApply = (script.physicsApplyForceObjectSpace || body)
        .getTransform()
        .getWorldTransform()
        .multiplyDirection(forceToApply)
        .normalize()
        .uniformScale(magnitude);
      break;
  }
...
function triggerSceneManagerLoadScene() {
  ...
  if (script.sceneManagerLoadSceneCallbackOnProgress) {
    ...
    onProgress =
      script.sceneManagerLoadSceneOnProgressCallbackScriptComponent[
        script.sceneManagerLoadSceneOnProgressCallbackFunctionName
      ];
  }
  var onComplete = null;
  if (script.sceneManagerLoadSceneCallbackOnComplete) {
    ...
    onComplete =
      script.sceneManagerLoadSceneOnCompleteCallbackScriptComponent[
        script.sceneManagerLoadSceneOnCompleteCallbackFunctionName
      ];
  }
