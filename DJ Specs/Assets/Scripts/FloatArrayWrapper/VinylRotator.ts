...
export class VinylRotator extends BaseScriptComponent {
  ...
  // --- DEBUG: always-on instrumentation (remove once verified) ---
  ...
  setOnDeck(state: boolean): void {
    print(`[DJDebug] VinylRotator(${this.propertyName}).setOnDeck(${state}) — calling ${this.trackPropertyName}() on ${this.audioControllers.length} controller(s)`);
  ...
  @bindUpdateEvent
  onUpdate(): void {
    const driftDistance = this.transform.getWorldPosition().distance(this.DEFAULT_POSITION);
    if (this.vinyl.enabled && driftDistance > 30) {
      print(`[DJDebug] VinylRotator(${this.propertyName}) AUTO-EJECT: vinyl drifted ${driftDistance.toFixed(1)}cm from awake-time deck position — setOnDeck(false), audio cleared`);
      this.vinyl.enabled = false;
      this.isOnDeck = false;
      this.audioControllers.forEach((controller) => {
    ...
    // DEBUG: 1 Hz while on deck. isTracking=true means the playback rate is
    // driven by hand motion (a still hand → rate≈0 → silence by design).
    if (getTime() - this.debugLastStatusTime >= 1.0) {
      this.debugLastStatusTime = getTime();
      print(`[DJDebug] rotator(${this.propertyName}): isTracking=${handCtrl.isTracking} paused=${this.isPaused} rate*speed=${(rate * this.speed).toFixed(3)}`);
    }
