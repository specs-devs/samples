...
export class MusicObject extends BaseScriptComponent {
  ...
  private _audioTrack: AudioTrackAsset
  ...
  onStart(): void {
    ...
    // Initialize button icons - start with play icon visible, pause icon hidden
    this._updateButtonIcons(false)
    this._playButton.initialize()
  ...
  /**
   * Set a downloaded AudioTrackAsset (Snap Cloud path) instead of inline base64 PCM.
   */
  setAudioTrack(audioAsset: AudioTrackAsset) {
    this._audioTrack = audioAsset
    this._hasAudio = true
    this._snap3DObject.setSpinnerEnabled(false)
    this._setPlayEnabled(true)
    // Reset to play state when new audio is set
    ...
    this._updateButtonIcons(false)
    if (this._musicPlayer) {
      this._audioDuration = this._musicPlayer.setTrack(audioAsset)
    }
  }
  ...
  private _onPlayClicked() {
    if (!this._hasAudio || !this._musicPlayer || (!this._audioTrack && !this._decodedAudio)) {
      return
    }
    if (this._isPlaying) {
      // Pause the audio
      if (this._audioTrack) {
        this._musicPlayer.pauseTrack()
      } else {
      ...
      // Play the audio
      if (this._audioTrack) {
        this._musicPlayer.playTrack()
      } else {
        this._musicPlayer.playAudio(this._decodedAudio)
      }
      this._isPlaying = true
      this._updateButtonIcons(true)
      // Track playback finish is reported by the AudioComponent; the timer is only
      // needed for the raw PCM path (and would fire early after a pause/resume)
      if (!this._audioTrack && this._audioDuration > 0) {
        if (!this._finishTimer) {
  ...
  private _onCloseClicked() {
    ...
    // Disable first so SIK's cursor drops its hover target, then destroy a tick
    // later - destroying a hovered Interactable mid-update makes the cursor's
    // InteractionManager throw "Object is null" every frame. setTimeout runs on
    // SIK's own dispatcher, so it still fires with this object disabled.
    try {
      this.sceneObject.enabled = false
    } catch (e) {
      this.logger.error(`Error disabling before destroy: ${e}`)
    }
    setTimeout(() => {
      try {
        if (this.sceneObject) {
