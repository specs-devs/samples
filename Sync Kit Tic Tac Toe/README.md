...
This is a sample [Connected Lens](https://developers.snap.com/spectacles/about-spectacles-features/connected-lenses/overview) project that demonstrates multiplayer gameplay using [Spectacles Sync Kit](https://developers.snap.com/spectacles/spectacles-frameworks/spectacles-sync-kit/getting-started). Two players take turns placing X and O pieces on a 3D tic-tac-toe grid, with game state fully synchronized across devices via [storage properties](https://developers.snap.com/spectacles/spectacles-frameworks/spectacles-sync-kit/features/storage-properties) and [networked events](https://developers.snap.com/spectacles/spectacles-frameworks/spectacles-sync-kit/features/networked-events). The sample also showcases the [Instantiator](https://developers.snap.com/spectacles/spectacles-frameworks/spectacles-sync-kit/features/instantiator) for spawning synced prefabs and SIK for piece manipulation.
...
[Controller.ts](./Assets/Scripts/Controller.ts) - Controls player role assignment and spawning of X and O piece prefabs using the Instantiator component.

[Piece.ts](./Assets/Scripts/Piece.ts) - Attached to each spawned piece prefab; enables manipulation for the player whose turn it is and handles snapping and win animations.

[GameManager.ts](./Assets/Scripts/GameManager.ts) - Manages turn logic, 3D win detection across all lines, and score tracking via storage properties.

[SyncEntityGrid.ts](./Assets/Scripts/SyncEntityGrid.ts) - String-based flat representation of the game grid serialized and synced via a storage property.

[GridPosition.ts](./Assets/Scripts/GridPosition.ts) - Defines the 3D coordinate of each grid snap point and positions it in local space.

## Testing the Lens

### In Lens Studio Editor

Open two Preview panels to test with multiple users in the Connected Lens. Each Preview simulates a user in the multiplayer session. X and O objects will spawn for each user when it is their turn. Use your mouse to move and arrange the objects to play a game of Tic Tac Toe.
