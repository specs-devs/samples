...
export class ModelNode extends BaseScriptComponent {
  ...
  public setNodeData(title: string, content: string, modelPrompt?: string): void {
    ...
    if (this.enableDebugLogging) {
      print(
        `ModelNode: Updated node ${this.nodeId} - Title: "${this.title}", Content: "${this.content.substring(0, 30)}..."`
      )
    }
  }
  ...
  public async generateContent(prompt: string): Promise<void> {
    ...
    if (!this.loadingSpinner) {
      print(`ModelNode: Loading spinner not assigned for node ${this.nodeId}`)
    }
        ...
        this.modelGenerator.setExternalCallbacks(
          () => {
            ...
            if (this.loadingSpinner) {
              ...
              print(`ModelNode: Loading spinner disabled - model received for node ${this.nodeId}`)
            }
          },
          (error: string) => {
            ...
            if (this.loadingSpinner) {
              this.loadingSpinner.enabled = false
              print(`ModelNode: Loading spinner disabled - error occurred for node ${this.nodeId}: ${error}`)
            }
          }
        )
    ...
    // Direct generation without scheduling
    {
      ...
      try {
        ...
        if (this.modelGenerator.getSceneObject()) {
          ...
          print(`ModelNode: ModelGenBridge scene object not found for node ${this.nodeId}`)
        }
        ...
        if (this.loadingSpinner) {
          this.loadingSpinner.enabled = false
          print(`ModelNode: Loading spinner disabled - generation error for node ${this.nodeId}`)
        }
