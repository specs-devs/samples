import {DiagramConnection, DiagramNode, DiagramState, DiagramUpdateResult} from "../Agents/AgentTypes"
...
export class DiagramUpdaterTool {
  ...
  ): Promise<{success: boolean; result?: DiagramUpdateResult; error?: string}> {
    try {
      const {
        analysis,
        currentDiagram,
      ...
      switch (updateType) {
        ...
        case "add_connection":
          updateResult = await this.addConnectionsFromAnalysis(analysis, currentDiagram as DiagramState)
          break

        case "restructure":
          updateResult = await this.restructureDiagram(analysis, currentDiagram as DiagramState)
          break
      ...
      print(
        `DiagramUpdaterTool: Diagram update complete - ${updateResult.updatedNodes.length} nodes, ${updateResult.newConnections.length} connections`
      )
  ...
  private async addNodesFromAnalysis(
    ...
    const newContent = allContent.filter((content) => !existingContent.includes(content.toLowerCase()))
   ...
   */
  private async addConnectionsFromAnalysis(analysis: any, currentDiagram: DiagramState): Promise<DiagramUpdateResult> {
    const newConnections: DiagramConnection[] = []
    ...
    if (analysis.educationalConnections) {
      for (const connection of analysis.educationalConnections) {
        ...
        const toNode = currentDiagram.nodes.find((n) => n.content.toLowerCase().includes(connection.to.toLowerCase()))
   ...
   */
  private async restructureDiagram(analysis: any, currentDiagram: DiagramState): Promise<DiagramUpdateResult> {
