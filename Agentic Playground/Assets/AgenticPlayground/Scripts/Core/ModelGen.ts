...
export class ModelGen extends BaseScriptComponent {
  ...
  onAwake() {
    ...
    if (!this.targetPosition) {
      print("ModelGen: Warning - No target position assigned, will use world origin")
    }
  }
        ...
        .then((submitGetStatusResults) => {
          submitGetStatusResults.event.add(([value, assetOrError]) => {
            if (value === "image") {
              ...
              if (this.enableDebugLogging) {
                print("ModelGen: Received preview image")
              }
  ...
  public setGenerationSettings(refineMesh: boolean, useVertexColor: boolean): void {
    ...
    if (this.enableDebugLogging) {
      print(`ModelGen: Settings updated - Refine: ${refineMesh}, VertexColor: ${useVertexColor}`)
    }
  }
