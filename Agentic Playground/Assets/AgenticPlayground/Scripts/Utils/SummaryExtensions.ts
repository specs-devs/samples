...
export class SummaryExtensions {
   ...
   */
  public static addSummaryCard(
    summaryLayout: SummaryComponent,
    summarySection: SummarySection,
    index: number
  ): boolean {
    if (!summaryLayout || !summarySection) {
      print(
        `SummaryExtensions: Invalid parameters - summaryLayout: ${!!summaryLayout}, summarySection: ${!!summarySection}`
      )
      return false
    }
    ...
    try {
      ...
      if (!layoutAny.testMode) {
        print(
          `SummaryExtensions: Dynamic mode detected. Current cards: ${layoutAny.cards ? layoutAny.cards.length : 0}, Requested index: ${index}`
        )
        ...
        if (!layoutAny.cards || index >= layoutAny.cards.length) {
          ...
          if (layoutAny.createDynamicCard && typeof layoutAny.createDynamicCard === "function") {
            print(
              `SummaryExtensions: Calling createDynamicCard with title: "${summarySection.title.substring(0, 50)}..."`
            )
  ...
  public static isReady(summaryLayout: SummaryComponent): boolean {
    ...
    try {
      const layoutAny = summaryLayout as any
      return layoutAny.initialized === true && layoutAny.cards !== null && layoutAny.cards !== undefined
    } catch (error) {
