...
export class GenerationQueue {
  ...
  public setImageGenerator(generator: (prompt: string) => Promise<any>): void {
    ...
    if (this.enableDebugLogging) {
      print("GenerationQueue: Image generator function registered")
    }
  }
  ...
  private async processModelQueue(): Promise<void> {
    if (this.enableDebugLogging) {
      print(
        `GenerationQueue: processModelQueue called - Queue: ${this.modelQueue.length}, Active: ${this.activeModelRequests}, Max: ${this.maxConcurrentModel}`
      )
    }
    ...
    if (this.modelQueue.length === 0 || this.activeModelRequests >= this.maxConcurrentModel) {
      if (this.enableDebugLogging) {
        print(
          `GenerationQueue: Skipping model processing - Queue empty: ${this.modelQueue.length === 0}, At capacity: ${this.activeModelRequests >= this.maxConcurrentModel}`
        )
      }
      return
    }
    ...
    while (this.modelQueue.length > 0 && this.activeModelRequests < this.maxConcurrentModel) {
      ...
      if (this.enableDebugLogging) {
        print(
          `GenerationQueue: Processing model request "${request.id}" - Active: ${this.activeModelRequests}/${this.maxConcurrentModel}, Remaining in queue: ${this.modelQueue.length}`
        )
      }
    ...
    if (this.enableDebugLogging) {
      print(
        `GenerationQueue: Model processing loop ended - Queue: ${this.modelQueue.length}, Active: ${this.activeModelRequests}`
      )
    }
    ...
    if (this.modelQueue.length > 0 || this.activeModelRequests > 0) {
      if (this.enableDebugLogging) {
        print(
          `GenerationQueue: Scheduling next model processing - Queue: ${this.modelQueue.length}, Active: ${this.activeModelRequests}`
        )
      }
      setTimeout(() => this.processModelQueue(), 100)
    } else {
  ...
  public clearQueues(): void {
    ...
    if (this.enableDebugLogging) {
      print("GenerationQueue: All queues cleared")
    }
  }
  ...
  public setConcurrencyLimits(imageLimit: number, modelLimit: number): void {
    ...
    if (this.enableDebugLogging) {
      print(
        `GenerationQueue: Concurrency limits set - Images: ${this.maxConcurrentImage}, Models: ${this.maxConcurrentModel}`
      )
    }
  }
