...
export class ModelGenerationScheduler {
  ...
  private async processQueue(): Promise<void> {
    if (this.isProcessing || this.activeGenerations >= this.maxConcurrent) {
      if (this.enableDebugLogging) {
        print(`ModelGenerationScheduler: Skipping processing - Active: ${this.activeGenerations}/${this.maxConcurrent}`)
      }
      return
    }
    ...
    while (this.queue.length > 0 && this.activeGenerations < this.maxConcurrent) {
      ...
      if (this.enableDebugLogging) {
        print(`ModelGenerationScheduler: Processing generation for node ${request.nodeId}`)
      }
  ...
  public setMaxConcurrent(max: number): void {
    ...
    if (this.enableDebugLogging) {
      print(`ModelGenerationScheduler: Max concurrent set to ${this.maxConcurrent}`)
    }
  }
  ...
  public clearQueue(): void {
    ...
    if (this.enableDebugLogging) {
      print("ModelGenerationScheduler: Queue cleared")
    }
  }
