...
export class Line3D extends BaseScriptComponent {
  ...
  @input
  @hint("Number of interpolated points between each path point for smooth curves")
  ...
  @input
  @hint("Use world position instead of local position (fixes most offset issues)")
  ...
  @input
  @hint("Apply additional coordinate space transformation relative to this component's transform")
  ...
  private generateMesh(): void {
    ...
    print("Line3D: Using " + (this._useWorldPosition ? "WORLD" : "LOCAL") + " coordinate space")
    if (this._zOffset !== 0) {
      print("Line3D: Applied Z-axis offset of " + this._zOffset + " to all path points")
    }
    ...
    if (this.meshBuilder.isValid()) {
      ...
      this.updateMaterial()
      print("Line3D: Mesh generated successfully with " + this.meshBuilder.getVerticesCount() + " vertices")
    } else {
  ...
  private getPathPositions(): vec3[] {
    if (this._debugPositions) {
      ...
      print("Line3D: Using " + (this._useWorldPosition ? "WORLD" : "LOCAL") + " positions")
    ...
    const originalPositions = this.pathPoints.map((point, index) => {
      ...
      let pos = this._useWorldPosition ? transform.getWorldPosition() : transform.getLocalPosition()
  ...
  private generateCircleVertices(center: vec3, right: vec3, up: vec3, vCoord: number): void {
    for (let i = 0; i < this._circleSegments; i++) {
      const angle = (i / this._circleSegments) * Math.PI * 2
      const cos = Math.cos(angle)
      const sin = Math.sin(angle)

      const localPos = right.uniformScale(cos * this._radius).add(up.uniformScale(sin * this._radius))
  ...
  private generateTubeIndices(pathLength: number): void {
    for (let segment = 0; segment < pathLength - 1; segment++) {
      for (let i = 0; i < this._circleSegments; i++) {
        ...
        const nextNext = (segment + 1) * this._circleSegments + ((i + 1) % this._circleSegments)

        this.meshBuilder.appendIndices([current, currentNext, next, next, currentNext, nextNext])
      }
    }
  }
  ...
  private generateEndCaps(pathPositions: vec3[]): void {
    ...
    const endForward = pathPositions[pathLength - 1].sub(pathPositions[pathLength - 2]).normalize()
  ...
  public setTestPositions(): void {
    ...
    print("Line3D: Interpolation steps = " + this._interpolationSteps + ", Smoothness = " + this._smoothness)
  ...
  public setManualOffset(offset: vec3): void {
    ...
    this.offsetCalculated = true
    print("Line3D: Manual offset set to (" + offset.x + ", " + offset.y + ", " + offset.z + ")")
    this.updateMesh()
  }
  ...
  public getPositionDiagnostics(): string {
    ...
    diagnostics += "- Use Relative to Component: " + this._useRelativeToComponent + "\n"
    ...
    if (this.pathPoints && this.pathPoints.length > 0) {
      diagnostics += "- Number of Path Points: " + this.pathPoints.length + "\n"
