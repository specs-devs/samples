...
const TAG = "InteractableLineProjection"
...
export class InteractableLineProjection extends BaseScriptComponent {
  ...
  methodName: string = "onScrollValueChanged"
  ...
  private onManipulationStarted(event: TransformEventArg): void {
    ...
    // Notify callback component about manipulation start
    if (this.callback && this.callback["onManipulationStart"]) {
      try {
        this.callback["onManipulationStart"]()
        this.log.d("Notified callback component about manipulation start")
      } catch (error) {
        this.log.e(`Error notifying manipulation start: ${error}`)
      }
    }
  }
  ...
  private onManipulationEnded(event: TransformEventArg): void {
    ...
    // Notify callback component about manipulation end
    if (this.callback && this.callback["onManipulationEnd"]) {
      try {
        this.callback["onManipulationEnd"]()
        this.log.d("Notified callback component about manipulation end")
      } catch (error) {
        this.log.e(`Error notifying manipulation end: ${error}`)
      }
    }
  }
  ...
  private calculateNormalizedValue(projectedPosition: vec3, lineStart: vec3, lineEnd: vec3): number {
    ...
    // Normalize to 0-1 range or 0-2 range based on doubleMapping
