...
import {isPlaceAlreadyTracked} from "./PlacesSearcher"

const NEARBY_PLACES_COUNT = 15
// Category taps filter client-side, so fetch a wider pool — with only the 15
// nearest places the filter often matches nothing and the tap looks dead.
const CATEGORY_PLACES_COUNT = 30
...
export class OutdoorNavigationPlaces extends BaseScriptComponent {
  ...
  private async searchNearbyPlaces(
    ...
    // Rapid taps launch overlapping async searches; without this guard both
    // pass cleanup and double-add their results.
    ...
    try {
      ...
      const fetchCount = categories !== null ? CATEGORY_PLACES_COUNT : NEARBY_PLACES_COUNT
      const nearby = await this.placesApi.getNearbyPlacesInfo(currentFocus, fetchCount, cacheThreshold)
      ...
      for (const place of results) {
        ...
        if (isPlaceAlreadyTracked(this.navigationModule, navPlace)) continue
        this.currentPlaces.push(navPlace)
        this.navigationModule.addPlace(navPlace)
      }

      if (categories !== null) {
        const label = categories[0].toLowerCase()
        this.panelManager.showStatus(
          results.length === 0
            ? `No ${label} places found nearby`
            : `Found ${results.length} ${label} place${results.length === 1 ? "" : "s"} nearby`
        )
      }
    } catch (e) {
      this.logger.error(`Error fetching nearby places: ${e}`)
      this.panelManager.showStatus("Couldn't search nearby places — check your connection")
    }
  }
