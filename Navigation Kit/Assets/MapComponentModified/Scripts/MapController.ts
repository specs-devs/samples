...
import {TWEEN_DURATION} from "MapComponentModified/MapUIController"
...
import {LocationAccuracyDisplay} from "../../Scripts/LocationAccuracyDisplay"
import {PlaceListCreator} from "../../Scripts/PlaceListCreator"
...
export class MapController extends BaseScriptComponent {
  ...
  private customUserLocation: GeoPosition | null = null
  private navigatingToPlace: Place | null = null
  ...
  initialize(
    ...
    if (this.mapParameters.setMapToCustomLocation) {
      ...
      this.customUserLocation = this.mapParameters.mapLocation
    }
  ...
  private onUpdate() {
    // userPosition can be undefined while NavigationDataComponent is still initializing.
    // Don't run map updates against a null position — wait for the next frame.
    if (isNull(this.userPosition) && !this.mapParameters.setMapToCustomLocation) {
      return
    }

    if (!this.isInitialized) {
      if (this.mapParameters.setMapToCustomLocation) {
        this.initializeWithFirstGeoPosition(this.mapLocation)
      } else {
    ...
    const location = this.mapParameters.setMapToCustomLocation
      ? this.customUserLocation
  ...
  private initializeWithFirstGeoPosition(location: GeoPosition): void {
    ...
    const effectiveLocation = this.mapParameters.setMapToCustomLocation
      ? this.mapLocation
      : location
    ...
    if (this.mapParameters.showUserPin) {
      this.spawnUserPin(this.mapParameters.userPinVisual, effectiveLocation, this.mapParameters.userPinScale)
    ...
    this.navigationComponent.onNavigationStarted.add((place) => {
      this.showOnlyPinForPlace(place)
    })

    this.isInitialized = true
    this.onUserLocationSetEvent.invoke(effectiveLocation)
  ...
  private updatePinLocation(pin: MapPin, position: GeoPosition): void {
    ...
    if (!isNull(this.navigatingToPlace)) {
      pin.setVisible(pin.place === this.navigatingToPlace)
    } else {
  ...
  private fromLocalPositionToLongLat(localPosition: vec2, zoomLevel: number): GeoPosition {
    ...
    return {longitude, latitude} as unknown as GeoPosition
  }
  ...
  private createMapGrid() {
    ...
    this.gridView = MapGridView.makeGridView(this, this.mapScaleX, this.mapScaleY)
  ...
  private showOnlyPinForPlace(place: Place | null): void {
    this.navigatingToPlace = place
    this.pinSet.forEach((p) => {
      p.setVisible(isNull(place) || p.place === place)
    })
  }
