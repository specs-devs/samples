...
/**
 * True if a place with the same name and (near-identical) coordinates is
 * already tracked by the navigation component. Different searchers create
 * distinct GeoLocationPlace instances for the same physical place, so the
 * navigation component's identity-based dedupe can't catch these.
 */
export function isPlaceAlreadyTracked(navigationModule: NavigationDataComponent, candidate: Place): boolean {
  return navigationModule.places.some((existing) => {
    if (existing.name !== candidate.name) return false
    const a = existing.getGeoPosition()
    const b = candidate.getGeoPosition()
...
export class PlacesSearcher extends BaseScriptComponent {
  ...
  private async searchCurrentMapPosition(addCenter: boolean = true): Promise<void> {
    // onUserPositionSet can fire multiple times in quick succession; without
    // this guard two in-flight searches both pass cleanup and double-add places.
    ...
    try {
      ...
      if (sequence !== this.searchSequence) return // superseded by a newer search
      nearby.forEach((p) => {
        const navigationPlace = this.createPlaceFromNearby(p)
        // Multiple searchers can be active (e.g. the guided-tour outdoor list
        // plus the map's own searcher) — don't add the same place twice.
        if (isPlaceAlreadyTracked(this.navigationModule, navigationPlace)) return
