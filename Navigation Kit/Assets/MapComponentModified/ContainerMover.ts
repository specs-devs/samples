...
import {delayAFrame} from "../Scripts/DelayAFrame"
...
export class ContainerMover extends BaseScriptComponent {
  ...
  onAwake() {
    this.logger = new Logger("ContainerMover", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  private async onStart() {
    ...
    this.container.followButton.onTriggerUp?.add(this.handleFollowButtonTrigger.bind(this))
    this.cameraTransform = WorldCameraFinderProvider.getInstance().getTransform()
    this.containerYOffset =
      this.container.getSceneObject().getTransform().getWorldPosition().y - this.cameraTransform.getWorldPosition().y + this.initialYOffset
  ...
  public setMoveable(movable: boolean, rotatable: boolean): void {
    // Frame uses useTagAlong for follow behavior (ContainerFrame.isFollowing).
    this.container.useTagAlong = movable
  ...
  private initializeSmoothFollow() {
    ...
    // Frame has no `border`/`constantPadding`; approximate width with innerSize + padding.
    this.resize(this.container.innerSize.x + this.container.padding.x * 2)
  ...
  finishDragging(): void {
    this.dragging = false
    this.containerYOffset = this.container.getSceneObject().getTransform().getWorldPosition().y - this.cameraTransform.getWorldPosition().y
    this.clampPosition()
  }
