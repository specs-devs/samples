// PinchButton/ToggleButton were removed from SIK 2.0. Pinch buttons now use the
// Interactable's onTriggerEnd directly; the toggle uses the SpectaclesUIKit
// Button in toggle mode (isOn + onValueChange). Inspector references must point
// at the Interactable / Button components on those objects.
...
export class MapUIController extends BaseScriptComponent {
  ...
  @input
  private spawnPinButton: Interactable
  private spawnPinButtonEnabled: boolean
  @input
  private clearPinsButton: Interactable
  private clearPinsButtonEnabled: boolean
  @input
  private zoomInButton: Interactable
  private zoomInButtonEnabled: boolean
  @input
  private zoomOutButton: Interactable
  private zoomOutButtonEnabled: boolean
  @input
  private centerMapButton: Interactable
  private centerMapButtonEnabled: boolean

  @input
  private toggleMiniMapButton: Button
  private toggleMiniMapButtonEnabled: boolean

  @input
  private showRestaurantsButton: Interactable
  private showRestaurantsButtonEnabled: boolean
  @input
  private showCafeButton: Interactable
  private showCafeButtonEnabled: boolean
  @input
  private showBarsButton: Interactable
  ...
  onAwake() {
    this.logger = new Logger("MapUIController", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  private onStart() {
    ...
    this.mapComponent.onMiniMapToggled.add((toggled) => {
      ...
      this.toggleMiniMapButton.isOn = toggled.isMini
  ...
  private handleToggleMiniMapButtonPinched(isOn: boolean) {
    ...
    this.logger.info("Toggling minimap " + isOn)
