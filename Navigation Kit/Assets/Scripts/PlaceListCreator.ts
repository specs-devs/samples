...
export class PlaceListCreator extends BaseScriptComponent {
  ...
  // ScrollView → SpectaclesUIKit ScrollWindow (re-add the component in scene).
  @input
  ...
  private spawnList(): void {
    ...
    // ScrollWindow recomputes its scroll bounds automatically when content
    // changes (ScrollView.recomputeBoundaries() has no direct equivalent).
  }
