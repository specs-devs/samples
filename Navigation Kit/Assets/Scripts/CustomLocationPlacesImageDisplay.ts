...
export class CustomLocationPlacesImageDisplay extends BaseScriptComponent {
  ...
  @allowUndefined @input private imageRoot: SceneObject
  @allowUndefined @input private image: Image
  @input private disableOnShow: SceneObject[]
  @input private manageEnabled = true
  @input public distanceToPrompt = 15

  public get visible(): boolean {
    return !isNull(this.imageRoot) && this.imageRoot.enabled
  }

  private onAwake(): void {
    this.createEvent("OnStartEvent").bind(() => {
      if (!isNull(this.image)) this.image.mainMaterial = this.image.mainMaterial.clone()
  ...
  public setVisible(visible: boolean): void {
    if (!isNull(this.imageRoot)) this.imageRoot.enabled = visible
  ...
  private showImage(placeTexture: CustomLocationPlaceTexturePair) {
    if (isNull(this.imageRoot) || isNull(this.image)) return
