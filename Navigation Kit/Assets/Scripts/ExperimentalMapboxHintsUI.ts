...
export class ExperimentalMapboxHintsUI extends BaseScriptComponent {
  ...
  private rebuild(update: HintsUpdate): void {
    ...
    // GridLayout 2.0 uses CSS-grid-style templates instead of maxRows/maxColumns/
    // cellWidth/cellHeight. Single column of fixed-width cells; auto-created rows
    // of fixed height. (Track string units may need tuning to match old layout.)
    this.gridLayout.templateColumns = `${BUTTON_WIDTH}cm`
    this.gridLayout.autoRows = `${BUTTON_HEIGHT}cm`
    this.gridLayout.columnGap = GRID_SPACING
    this.gridLayout.rowGap = GRID_SPACING
  ...
  private createStepRow(title: string, subtitle: string, titleColor: vec4): void {
    ...
    // GridLayout 2.0 only positions children that carry a GridItem component.
    btnObj.createComponent(GridItem.getTypeName())
