...
export class PlacesUIComponentScroll extends BaseScriptComponent {
  ...
  private flexContainer: SceneObject
  ...
  private start(): void {
    ...
    this.navigationComponent.onArrivedAtPlace.add((place) => {
  ...
  private rebuild(): void {
    ...
    // Match the ExampleScrollWindowLayout pattern: a vertical FlexLayout column
    // inside the ScrollWindow. Content height = sum of item heights + gaps.
    const itemPitch = BUTTON_HEIGHT + GRID_SPACING
    const contentHeight = places.length * itemPitch - GRID_SPACING
    const contentWidth = BUTTON_WIDTH
    ...
    ;(this.scrollWindow as any)._edgeFade = true

    this.flexContainer = global.scene.createSceneObject("PlacesFlex")
    this.flexContainer.setParent(this.scrollWindowObject)

    this.flexLayout = this.flexContainer.createComponent(
      FlexLayout.getTypeName()
    ) as FlexLayout
    // Items are added manually via addItems() (in createPlaceButton); disable
    // auto-discovery and configure inside onInitialized, per the example.
    this.flexLayout.autoDiscoverItemsOnStart = false
    this.flexLayout.onInitialized.add(() => {
      this.flexLayout.width = contentWidth
      this.flexLayout.height = contentHeight
      this.flexLayout.direction = FlexDirection.Column
      this.flexLayout.rowGap = GRID_SPACING
      this.flexLayout.justifyContent = FlexJustify.Start
      this.flexLayout.alignItems = FlexAlign.Stretch
    })

    const flexItems: FlexItem[] = []
    for (let i = 0; i < places.length; i++) {
      print(TAG + " creating button for place[" + i + "]: " + (places[i].name ?? "unknown"))
      const entry = this.createPlaceButton(places[i], flexItems)
  ...
  private createPlaceButton(place: Place, flexItems: FlexItem[]): PlaceEntry {
    const btnObj = global.scene.createSceneObject(place.name ?? "Place")
    btnObj.setParent(this.flexContainer)

    // FlexLayout positions children that carry a FlexItem with explicit size.
    const flexItem = btnObj.createComponent(FlexItem.getTypeName()) as FlexItem
    flexItem.overrideWidth = BUTTON_WIDTH
    flexItem.overrideHeight = BUTTON_HEIGHT
    flexItem.flexGrow = 0
  ...
  private applyZOffset(): void {
    ...
    for (let i = 0; i < this.flexContainer.getChildrenCount(); i++) {
      const child = this.flexContainer.getChild(i)
