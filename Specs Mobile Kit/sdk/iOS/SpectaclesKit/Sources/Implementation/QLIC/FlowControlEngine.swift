...
struct FlowControlEngine<Clock: ClockProtocol> {
    ...
    /**
     ...
     - parameter enforceCreditCap: When false, the cap is bypassed and the next packet is always sized at half the transmit estimate. Used by the handshake logic to prevent credit depletion deadlocks.
     */
    func nextPacketSize(enforceCreditCap: Bool) -> Int {
        guard enforceCreditCap else { return estimatedTransmitSize / 2 }
