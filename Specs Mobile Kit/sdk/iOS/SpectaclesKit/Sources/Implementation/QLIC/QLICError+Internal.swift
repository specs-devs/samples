...
extension QLICConnectionError {
    static func localProtocolError(
        errorCode: QLICProtocolErrorCode,
        reasonString: String,
        frameType: Int,
        ...
        .localProtocolError(
            ...
            reason: Data(reasonString.utf8),
            frameType: frameType,
            underlying: underlying
        )
    }

    static func cryptoError(errorCode: QLICProtocolErrorCode, reasonString: String, underlying: any Error) -> QLICConnectionError {
        .localProtocolError(errorCode: errorCode, reasonString: reasonString, frameType: 0x06, underlying: underlying)
    }

    var closeFrame: QLICFrame? {
        switch self {
        case .remoteAppError, .remoteProtocolError, .runtimeError:
            nil
        case let .localAppError(errorCode, reason):
            .applicationConnectionClose(applicationErrorCode: errorCode, reason: reason)
        case let .localProtocolError(errorCode, reason, frameType, _):
            .protocolConnectionClose(
