...
struct EncryptionEngine {
     ...
     */
    mutating func seal(payload: Data) throws(QLICConnectionError) -> [Data] {
        if txEncryptor == nil {
            return [payload]
        } else {
            do {
                ...
                throw .cryptoError(errorCode: .tlsInternalError, reasonString: "Encryption failed", underlying: error)
            }
        }
    }
     ...
     */
    mutating func open(payload: Data) throws(QLICConnectionError) -> Data {
        if rxDecryptor == nil {
            return payload
        } else {
            guard payload.count >= Self.tagLength else {
                throw .cryptoError(
                    errorCode: .tlsDecodeError,
                    reasonString: "Payload too short",
            ...
            do {
                ...
                throw .cryptoError(errorCode: .tlsDecryptError, reasonString: "Decryption failed", underlying: error)
            }
        }
    }
