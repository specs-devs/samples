...
extension AuthenticationShareData: VisitorEncodable {
    ...
    func encode(into visitor: inout some EncodingVisitor) throws {
        ...
        case .pretrusted:
            try visitor.visit(0x1)
        case let .iosDeviceCheck(publicKey: publicKey, attestationObject: attestationObject, appId: appId):
            try visitor.visit(0x2)
            try visitor.visit(publicKey.count)
            visitor.visit(publicKey)
            try visitor.visit(attestationObject.count)
            visitor.visit(attestationObject)
            let appIdData = Data(appId.utf8)
            try visitor.visit(appIdData.count)
            visitor.visit(appIdData)
    ...
    static func dequeueShareData(from data: inout Data) throws -> AuthenticationShareData {
        ...
        case 0x2:
            ...
            let appIdCount = try data.dequeueVarInt()
            let appIdData = try data.dequeueData(count: appIdCount)
            guard let appId = String(data: appIdData, encoding: .utf8) else {
                throw UTF8DecodingError()
            }
            return .iosDeviceCheck(publicKey: publicKey, attestationObject: attestationObject, appId: appId)
        case 0x4:
...
extension HandshakeRecord: VisitorEncodable {
    ...
    /// Parses a record from some data, removing and returning those bytes
    static func dequeueRecordAndData(from data: inout Data) throws -> (record: HandshakeRecord, encodedBytes: Data)? {
        var copy = data
        let record: HandshakeRecord
        do {
            record = try dequeueRecord(from: &copy)
        } catch is Data.TooSmallError {
            return nil
        }
        return try (record, data.dequeueData(count: data.count - copy.count))
    }
