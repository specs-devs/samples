...
actor BluetoothManager: QueueActor {
    ...
    static var spectaclesServiceUUID: CBUUID { CBUUID(string: "FE45") }
    ...
    func openL2CAPChannel(peripheralId: UUID, assignedId: String?) async throws -> UnsafeTransfer<CBL2CAPChannel> {
        guard case .running = invalidationState else { throw BluetoothError.invalidated }
     ...
     */
    private func getPSM(for peripheralId: UUID, assignedId: String?) async throws -> CBL2CAPPSM {
        try await waitForCentralManager()
            ...
            case .didDiscover(peripheralId: peripheralId, advertisementData: let advertisementData, rssi: _):
                ...
                if spectaclesServiceData.count == 18 {
                    if let assignedId {
                        ...
                        // UUID string equality is case-insensitive — device-issued ids are lowercase,
                        // `UUID.uuidString` renders uppercase, so we compare canonical forms.
                        guard assignedId.caseInsensitiveCompare(discoveredId.uuidString) == .orderedSame else {
