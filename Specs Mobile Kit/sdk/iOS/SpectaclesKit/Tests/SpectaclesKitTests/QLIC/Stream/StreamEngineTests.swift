...
final class StreamEngineTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        stream = try await engine.openStream(sendEmptyFirstFrame: false)
    }
    ...
    func testStreamChunksData() async throws {
        ...
        while !isClosed {
            do {
                let content = try await stream.receive(range: 300 ... 400)
                XCTAssert(content.count <= 400)
                receivedDataCount += content.count
                if content.count < 300 {
                    // Stream closed gracefully with partial content
                    isClosed = true
                }
            } catch QLICStreamError.streamClosed {
                isClosed = true
            } catch {
    ...
    func testStreamHandlesAppCloseErrors() async throws {
        ...
        do {
            _ = try await stream.receive(range: 1 ... 1000)
            XCTFail("Should throw after closed for reading")
        } catch QLICStreamError.streamAborted(_) {
            // expected
        } catch {
            XCTFail("Expected streamAborted, got \(error)")
            return
        }
    ...
    func testStreamHandlesPeerCloseErrors() async throws {
        ...
        do {
            _ = try await stream.receive(range: 1 ... 1000)
            XCTFail("Should throw after reset")
        } catch QLICStreamError.streamAborted(_) {
            // expected
        } catch {
            XCTFail("Expected streamAborted, got \(error)")
