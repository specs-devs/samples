...
final class QLICFrameCodingTests: XCTestCase {
    ...
    /// New-generation Spectacles emit crypto frames as either type 0x6 or 0x7; the decoder must accept both.
    func testCryptoFrameType0x7DecodesAsCrypto() throws {
        let sampleData = Data([0xAA, 0xBB, 0xCC, 0xDD])
        var buffer = Data([0x07, 0x04, 0xAA, 0xBB, 0xCC, 0xDD])
        try XCTAssertEqual(QLICFrame.dequeueFrame(from: &buffer), .crypto(cryptoData: sampleData))
