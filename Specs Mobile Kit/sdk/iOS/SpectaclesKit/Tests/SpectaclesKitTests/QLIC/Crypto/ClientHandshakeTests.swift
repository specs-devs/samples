...
final class ClientHandshakeTests: XCTestCase, @unchecked Sendable {
    ...
    func testClientHandshake() async throws {
        async let _: Void = {
            do {
                let secrets = try await clientEngine.runClientHandshake(
                ...
                try await clientEngine.handleKeyUpdates(txSecret: secrets.clientSecret, rxSecret: secrets.serverSecret)
            } catch {
                if !Task.isCancelled {
...
extension HandshakeEngine {
    func runUnauthenticatedServerHandshake(
        ...
        guard
            ...
            ) = try await receiveHandshakeRecord()
        else {
        ...
        try updateRxKey(clientHandshakeKey)
        ...
        guard case .keyUpdate(updateRequested: false) = try await receiveHandshakeRecord() else {
        ...
        try updateRxKey(updatedClientAppKey)
    }
    ...
    func runServerHandshake(authVerifier: any HandshakeAuthVerifier, authProvider: any HandshakeAuthProvider) async throws {
        ...
        try updateRxKey(handshakePeerSecrets.clientSecret.expandKeys(securityLevel: .handshake))
