...
final class FlowControlEngineTests: XCTestCase {
    ...
    func testPacketSizes() {
        let packetAlignment = 1253 // Half of initial estimated transmit size (2506)

        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == packetAlignment)
        engine.onPacketWritten(isAckSoliciting: true, count: packetAlignment - 1000, ackMetadata: nil)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == packetAlignment + 1000)
        engine.onPacketWritten(isAckSoliciting: true, count: packetAlignment + 1000, ackMetadata: nil)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == packetAlignment)
        engine.onPacketWritten(isAckSoliciting: true, count: packetAlignment, ackMetadata: nil)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == packetAlignment)
        engine.onPacketWritten(isAckSoliciting: true, count: packetAlignment, ackMetadata: nil)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == 0)

        _ = engine.onAckFrame(bytesSinceLastAck: 1000)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == 0)
        _ = engine.onAckFrame(bytesSinceLastAck: 1000)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == 2000)
        _ = engine.onAckFrame(bytesSinceLastAck: 1000)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == 3000 - packetAlignment)
        engine.onPacketWritten(isAckSoliciting: true, count: 3000 - packetAlignment, ackMetadata: nil)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == packetAlignment)
        engine.onPacketWritten(isAckSoliciting: true, count: packetAlignment, ackMetadata: nil)
        XCTAssert(engine.nextPacketSize(enforceCreditCap: true) == 0)

        // With the cap bypassed (pre-handshake), the engine always returns half the transmit estimate regardless of outstanding credit.
        XCTAssertEqual(engine.nextPacketSize(enforceCreditCap: false), packetAlignment)
    }
