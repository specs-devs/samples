...
export class BalloonManager extends BaseScriptComponent {
  ...
  private onStart(): void {
    ...
    this.balloons.forEach((obj, i) => {
      ...
      interactable.onInteractorTriggerEnd.add((_event: InteractorEvent) => {
  ...
  public changeTransform(): void {
    const transform = this.selectedBalloon.getTransform()
    const startPosition = transform.getLocalPosition()
    const destinationPosition = new vec3(startPosition.x, 50, startPosition.z)
    // LSTween was removed; native SIK animate (durations in seconds, ms before).
    animate({
      duration: 1.5,
      easing: "ease-in-out-cubic",
      update: (t: number) => {
        transform.setLocalPosition(vec3.lerp(startPosition, destinationPosition, t))
      },
    })
